...
STT_INITIAL_PROMPT  = (
    "هذا تسجيل صوتي بالدارجة التونسية لعميل يتصل بتليكوم تونس. "
    "العميل يصف مشكلته بالكلمات الشائعة في خدمة العملاء. "
    # ── مشكلة في الدفع ────────────────────────────────────────
    "مشكلة في الدفع، ما خلصتش، الدفع ما مشاش، عندي مشكلة في الخلاص، "
    "MyTT ما تقبلش، حوالة بنكية، مشكلة في التحويل. "
    # ── اعتراض على الفاتورة ───────────────────────────────────
    "اعتراض على الفاتورة، الفاتورة غالية، ما هي صحيحة، فاتورة غير عادلة، "
    "مبلغ زيادة، نعترض على الفاتورة، الفاتورة مش صح. "
    # ── انقطاع الانترنات ──────────────────────────────────────
    "انقطاع الانترنت، قطع عندي الانترنت، الإنترنت ما يخدمش، "
    "باكس ما يتصلش، ريزو ما يجيش، وايفاي مقطوع، ما عندي انترنت. "
    # ── تأخير في التركيب ──────────────────────────────────────
    "تأخير في التركيب، التقني ما جاش، موعد التركيب فات، "
    "طلبت تركيب مش جا، التقني متأخر. "
    # ── تغيير الخدمة ──────────────────────────────────────────
    "تغيير الخدمة، نبدل الباقة، نرفع الاشتراك، نغير الفورفي. "
    # ── كلمات عامة darija ─────────────────────────────────────
    "عسلامة، ياسر، برشا، مش، ميفوتش، بوجار، الريزو، الموديم، "
    "الفاتورة، الرصيد، الموبيل، الإنترنت. "
    # ── villes / wilayas ──────────────────────────────────────
    "المنستير، سوسة، صفاقس، تونس، قصر هلال، بنزرت، نابل، "
    "قابس، قفصة، سيدي بوزيد، القيروان، أريانة، بن عروس."
)
...
DELEGATION_QUESTION = "في أي معتمدية بالضبط في {wilaya}؟"

# Demande de localisation complète (ni wilaya ni délégation connues)
LOCATION_QUESTION = "قولي من أي ولاية ومعتمدية أنت؟ هذا ضروري باش نعاونك بطريقة أحسن."

# Demande de délégation seule (wilaya déjà connue, délégation manquante)
DELEGATION_ONLY_QUESTION = "في أي معتمدية بالضبط؟"

# Message demandant le numéro de demande/ticket avant transfert ou réponse apprise.
# Utilisé pour : مشكلة في الدفع, اعتراض على الفاتورة, انقطاع الانترنات, تأخير في التركيب
ASK_REQUEST_NUMBER_MSG = "أعطيني رقم الطلب أو المعاملة باش نكمل معك."

# Intents qui nécessitent la localisation pour répondre correctement
# (pannes réseau, installation, couverture, échange matériel…)
LOCATION_DEPENDENT_INTENTS: set = {
    # Réseau / connexion
    "انقطاع الانترنات", "انقطاع الانترنت",
    "عطل في الشبكة",   "عطل في الشبكه",
    "بطء في الانترنات", "بطء في الانترنت",
    "مشكلة في إشارة الويفي", "مشكله في اشاره الويفي",
    # Installation / couverture
    "تأخير في التركيب", "تاخير في التركيب",
    "استفسار عن التغطية", "استفسار عن التغطيه",
    # Appareil / agence physique
    "عطب في الجهاز",
    "تبديل شريحة",     "تبديل شريحه",
    # Itinérance
    "مشكلة في التجوال", "مشكله في الجوال",
}
...
THANKS_KEYWORDS = [
    # ── عربي فصحى ─────────────────────────────────────────────────
    "شكرا", "شكراً", "شكرًا", "شكر", "شكرًا جزيلاً", "شكرا جزيلا",
    "أشكرك", "اشكرك", "جزاك الله خيرا", "جزاك الله", "بارك الله فيك",
    "بارك الله", "يبارك فيك", "الله يبارك فيك",
    # ── دارجة تونسية ──────────────────────────────────────────────
    "شكرن", "مشكور", "مشكورين",
    "مرسي", "ميرسي", "مارسي",
    "يعيشك", "عيشك", "يعيش",
    "ربي يحفظك", "ربي يخليك", "ربي يعطيك الصحة",
    "برابر", "مزيان", "مليح", "واعر",
    # ── يعطيك الصحة (variantes orthographiques complètes) ─────────
    "يعطيك الصحة",  "يعطيك صحة",
    "يعطيك الصحه",  "يعطيك صحه",
    "يعطيك",        "عطيك الصحة",   "عطيك صحة",
    "يعطيك العافية","يعطيك عافية",
    "يعطيكم الصحة", "يعطيكم صحة",
    # ── expressions complètes fréquentes ──────────────────────────
    "الله يجازيك", "الله يعطيك", "الله يخليك",
    "ربي يجازيك", "ربي يعطيك",
    "تسلم", "تسلمو", "تسلمولي", "سلامتك",
    "حمدالله", "الحمد لله", "الحمد",
    # ── français ──────────────────────────────────────────────────
    "merci", "merci beaucoup", "merci bien",
    "parfait", "excellent", "super",
    "bravo", "bravoo", "chapeau",
    "c'est bon", "nickel", "ok merci",
]

THANKS_MESSAGE = (
    "يعطيك الصحة ونشكرك على ثقتك في تليكوم تونس! "
    "يسعدنا خدمتك — مرسي برشا على تواصلك معنا. "
    "إذا عندك أي مشكلة أخرى، إحنا هنا دايماً نعاونك. "
    "يوم سعيد وربي يحفظك!"
)
